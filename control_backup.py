...
def controllaw(sim, robot, trajs, tcurrent, cube, cube_traj=None):
    """
    ...
    - Force control to push/hold the cube following the planned trajectory

    Key: The cube should move due to PHYSICAL contact forces from robot hands,
    not by manually positioning it. The robot follows a trajectory while applying
    forces to carry the cube.
    """
    ...
    try:
        ...
        # Get DESIRED cube position from planned trajectory (not actual PyBullet position!)
        # This is the key fix - we want robot to push cube to follow the plan
        if cube_traj is not None:
            cube_placement_desired = cube_traj(tcurrent)
        else:
            # Fallback: use current PyBullet position (less ideal)
            cube_pos_pybullet, cube_orn_pybullet = pyb.getBasePositionAndOrientation(sim.cubeId)
            cube_placement_desired = pin.XYZQUATToSE3(np.array([
        ...
        # Get DESIRED hook positions based on PLANNED cube trajectory
        # This makes the robot actively push the cube to the desired location
        setcubeplacement(robot, cube, cube_placement_desired)

        # Get hook positions in world frame (where hands SHOULD be)
        ...
        # This creates forces that push hands toward desired grasp points
        # When hands contact the cube, these forces are transmitted to the cube
        Kf = 500.0  # Force gain - increased for better grasping
    ...
    # PyBullet automatically handles contact forces when hands touch cube
    sim.step(torques)
...
if __name__ == "__main__":
    ...
    # Create cube trajectory function from planned cube waypoints
    # This interpolates the desired cube position over time
    def make_cube_traj(cube_waypoints, T):
        """Create cube trajectory function from waypoints"""
        ...
        def cube_traj(t):
            if t <= 0:
                ...
                # Keep rotation constant (cube doesn't rotate during grasp)
    ...
    if VISUALIZE_ONLY:
        ...
        print("NOTE: In this preview, cube is moved manually for visualization.")
        print("      In actual simulation, cube moves through physical contact.")
        ...
        for i in range(n_frames + 1):
            ...
            # Get cube position from trajectory (for visualization only!)
            # In actual PyBullet simulation, cube moves due to contact forces
        ...
        # dynamics simulation with PHYSICAL grasping
        print("\n" + "="*70)
        print("Running control simulation with physics...")
        print("The cube will move due to contact forces from robot hands.")
        print("This will take a while (real-time simulation)...")
        print("="*70 + "\n")

        tcur = 0.

        # Create a wrapper function that includes cube_traj
        def controllaw_wrapper(sim, robot, trajs, tcur, cube):
            controllaw(sim, robot, trajs, tcur, cube, cube_traj=cube_traj)
